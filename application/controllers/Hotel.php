...
class Hotel extends CI_Controller
{
	...
	public function loginsubmit()
	{
		if (IS_AJAX) {
			...
			if ($this->form_validation->run()) {
				...
				$exist = $this->db->select("*")->from('customerinfo')
					->where('email', $email)
					->where('pass', $password)
					->get()->row();
				...
				if (!empty($exist)) {
					...
					$this->session->set_userdata($sessiondata);
					$this->load->helper('generate_jwt_token_helper');
					...
					return $this->output
						...
						->set_output(json_encode([
							...
							'token' => generate_jwt_token($sessiondata)
						]));
				} else {
					return $this->output
						->set_content_type('application/json')
				...
				return $this->output
					...
					->set_output(json_encode([
						'err' => 1,
						'success' => false,
						'message' => "Invalid JSON data: " . validation_errors()
					]));
			}
		} else {
			// Xử lý nếu không phải là request AJAX (thông thường)
			$this->form_validation->set_rules('email', display('email'), 'required');
			$this->form_validation->set_rules('password', display('password'), 'required');
	...
	public function signup()
	{
		if (IS_AJAX) {
			...
			// );
			if ($this->form_validation->run()) {
				$f_name = $json_data['f_name'] ?? null;
				$l_name = $json_data['l_name'] ?? null;
				$phone = $json_data['phone'] ?? null;
				$emial = $json_data['emial'] ?? null;
				$password = isset($json_data['password']) ? md5($json_data['password']) : null;
	...
	public function smart_door()
	{
		$json_data = json_decode($this->input->raw_input_stream, true);
		$this->db->select('lock_status, locked_by_admin')
			->from('tbl_floorplan')
			->where([
				'smart_lock_mac' => $json_data['mac_address'],
				'floorplanid' => $json_data['room_id']
			]);
		$query = $this->db->get();
		$room = $query->row();
		...
		if ($room->locked_by_admin) {
			return $this->output
				->set_content_type('application/json')
				->set_status_header(200)
				->set_output(json_encode([
					'success' => false,
					'message' => 'This lock is controlled by the admin and cannot be toggled.'
				]));
		}

		$this->db->set('lock_status', $json_data['new_status']);
		$this->db->where('floorplanid', $json_data['room_id']);
		...
		if ($this->db->affected_rows() > 0) {
			...
			$esp32_url = "http://esp32-ip-address/control-lock";
			$postData = http_build_query([
				'room_id' => $json_data['room_id'],
				'lock_status' => $json_data['new_status']
			]);
			$options = [
				'http' => [
			...
			if ($result === FALSE) {
				$response['esp32_message'] = 'Failed to communicate with ESP32.';
			} else {
				$response['esp32_message'] = 'ESP32 responded successfully.';
			}
		} else {
			$response = [
				'success' => false,
	...
	public function controlSmartLock()
	{
		$json_data = json_decode($this->input->raw_input_stream, true);
		$command = $json_data['command'];
		$room_id = $json_data['room_id'];
		$mac_address = $json_data['mac_address'];
		if (!in_array($command, ['unlock', 'lock'])) {
			return $this->output
				->set_content_type('application/json')
		...
		$this->db->select('lock_status, locked_by_admin, esp32_url, floorplanid, smart_lock_mac')
			->from('tbl_floorplan')
			->where([
				'floorplanid' => $room_id,
				'smart_lock_mac' => $mac_address,
			]);
		$query = $this->db->get();
		$room = $query->row();
		if (!$room) {
		...
		if ($room->floorplanid != $room_id || $room->smart_lock_mac != $mac_address) {
			return $this->output
				->set_content_type('application/json')
				->set_status_header(404)
				->set_output(json_encode([
		...
		$postData = http_build_query(['command' => $command]);
		$options = [
			'http' => [
				'header' => "Content-Type: application/x-www-form-urlencoded\r\n",
				'method' => 'POST',
		...
		$context = stream_context_create($options);
		// $result = file_get_contents($room->esp32_url, false, $context);
		$result = true;
		...
		if ($result === FALSE) {
			...
			$new_lock_status = $command === 'lock' ? '1' : '0';
			$this->db->where('floorplanid', $room_id);
			$this->db->update('tbl_floorplan', ['lock_status' => $new_lock_status]);
			return $this->output
				->set_content_type('application/json')
				->set_status_header(200)
				->set_output(json_encode([
	...
	{
		$json_data = json_decode($this->input->raw_input_stream, true);
		$device_id = $json_data['mac_address'];
		$floorplan_id = $json_data['floorplan_id'];
		$command = $json_data['command'];
		if (empty($device_id)  || empty($floorplan_id)) {
			return $this->output
				->set_content_type('application/json')
				->set_status_header(200)
				->set_output(json_encode([
		...
		$query = $this->db->get();
		$room = $query->row();
		if ($room->locked_by_admin == 1) {
			return $this->output
				->set_content_type('application/json')
				->set_status_header(200)
				->set_output(json_encode([
					'success' => false,
					'message' => 'Device lock by admin'
				]));
		}

		$data = [
			'lock_status' => $command == 1 ? '1' : '0'
		];
		$this->db->where('floorplanid', $floorplan_id);
		$this->db->update('tbl_floorplan', $data);
		return $this->output
			->set_content_type('application/json')
			->set_status_header(200)
			->set_output(json_encode([
	...
	public function registerDevice()
	{
		...
		if (!empty($esp32_url)) {
			$data['esp32_url'] = $esp32_url;
		}
		
		if (!empty($floorplan_id)) {
			$data['floorplanid'] = $floorplan_id;
		}
		
		if (!empty($device_id)) {
			$data['smart_lock_mac'] = $device_id;
		}
		$this->db->where('floorplanid', $floorplan_id);
		$this->db->update('tbl_floorplan', $data);
		if ($query->num_rows() > 0) {
