...
class Roomdetails_model extends CI_Model
{
	...
	public function updateFacilities($roomId, $facilities)
	{
		$facilitiesJson = json_encode($facilities);
		$this->db->where('room_id', $roomId)
			->update('roomdetails', ['facilities' => $facilitiesJson]);
	}

	public function getFacilities($roomId)
	{
		$data = $this->db->select('facilities')
			->from('roomdetails')
			->where('room_id', $roomId)
			->get()
			->row();
		return json_decode($data->facilities, true);
	}
